...
{
    public partial class TCSConfigList
    {
        [Inject] private TCSConfigService ConfigService { get; set; } = default!;
        [Inject] private ToastService ToastService { get; set; } = default!;

        private List<TCSConfigListDto> AllItems { get; set; } = new();
        private List<TCSConfigListDto> FilteredItems { get; set; } = new();
        private List<TCSConfigListDto> PagedItems { get; set; } = new();
        private TCSConfigListDto? SelectedItem;
        ...
        {
            searchText = SelectedStatus = SelectedSection = SelectedContext = string.Empty;
            CurrentPage = 1;
            AllItems = await ConfigService.GetAllAsync();
            ApplyFilters();
        }
        ...
        private void ApplyFilters()
        {
            ...
            if (!string.IsNullOrEmpty(SelectedContext) && int.TryParse(SelectedContext, out var ci))
                q = q.Where(x => (int)x.TransactionContext == ci);
        ...
        private void OpenViewModal(TCSConfigListDto item) => SelectedItem = item;
